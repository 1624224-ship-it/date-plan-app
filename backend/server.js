...
スポットの時刻は必ず現実的な移動時間＋15分以上のバッファを考慮してください（例：電車移動なら乗換・歩行を含む）。
各スポットのbudgetは必ず1以上の実際の費用（入場料・体験料・ドリンク代など）を整数で入れてください。無料のスポットは0ではなく、周辺での飲食や交通費の目安を入れてください。
...

{
  ...
  "lunch_options": [
    {
      "name": "そのエリアで人気の地元店・デートにぴったりな雰囲気の良い店名（チェーン店は避ける）",
      "genre": "ジャンル（例：イタリアン、フレンチ）",
      "price_per_person": 数値,
      "memo": "デートに合うおすすめポイント（雰囲気・料理の特徴など）"
    }
  ],
  "dinner_options": [
    {
      "name": "そのエリアで人気の地元店・デートにぴったりな雰囲気の良い店名（チェーン店は避ける）",
      "genre": "ジャンル（例：焼肉、フレンチ）",
      "price_per_person": 数値,
      "memo": "デートに合うおすすめポイント（雰囲気・料理の特徴など）"
    }
  ]
